...
const Firefly = () => {
  ...
  return (
    <motion.div 
      animate={{ x: [0, Math.random() * 60 - 30, 0], y: [0, Math.random() * 60 - 30, 0], opacity: [0.1, 0.6, 0.1], scale: [1, 1.4, 1] }} 
    ...
    >
      <AnimatePresence>
        {isSelected && !participant.isMe && ( 
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: [1, 1.25, 1], opacity: [0.4, 0.8, 0.4] }} exit={{ scale: 0.8, opacity: 0 }} transition={{ repeat: Infinity, duration: 2 }} className="absolute rounded-full border-2 border-dashed border-pink-400/50" style={{ width: bubblePx + 24, height: bubblePx + 24, top: -12, left: -12 }} /> 
        )}
      </AnimatePresence>
      {participant.playingAudio && ( 
        <motion.div animate={{ scale: [1, 2.5, 1], opacity: [0.6, 0, 0.6] }} transition={{ duration: 0.8, repeat: Infinity }} className="absolute rounded-full bg-pink-400/40" style={{ width: bubblePx + 30, height: bubblePx + 30, top: -15, left: -15 }} /> 
      )}
      {participant.mood && ( 
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-4 -right-2 bg-white/90 backdrop-blur-sm shadow-xl rounded-full w-6 h-6 flex items-center justify-center text-xs border border-pink-100 z-40">
          {participant.mood}
        </motion.div> 
      )}
      <motion.div whileHover={{ scale: 1.1, rotate: [0, -5, 5, 0] }} animate={{ width: bubblePx, height: bubblePx }} transition={{ type: 'spring' }} className="rounded-full flex items-center justify-center shadow-2xl border-4 border-white/40 ring-4 ring-pink-500/10 overflow-hidden relative bg-gradient-to-br from-pink-300 via-rose-500 to-rose-700">
        ...
        {participant.hasUnplayedAudio && ( 
          <div className="absolute top-2 right-2 w-3 h-3 bg-red-50 border-2 border-white rounded-full animate-bounce" /> 
        )}
      </motion.div>
...
const RadioDasMaes = () => {
  ...
  const [myName] = useState(() => {
    try { 
      const p = localStorage.getItem('almas_empreendedora_profile'); 
      return p ? JSON.parse(p).nomeEmpreendedora || 'Mãe' : 'Mãe'; 
    } catch { return 'Mãe'; }
  });
  ...
  return (
    <div className="w-full max-w-6xl mx-auto h-[calc(100vh-100px)] flex flex-col pt-2 pb-6 px-4 overflow-hidden bg-transparent">
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'podcast' ? (
          <motion.div key="stage-area" className="flex-1 flex flex-col gap-3 min-h-0 relative">
              ...
              <motion.div style={{ rotateX, rotateY, transformStyle: 'preserve-3d' }} className="absolute inset-0 w-full h-full flex items-center justify-center pointer-events-none">
                <div className="absolute inset-0 pointer-events-auto">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 z-50">
                        ...
                        <button onClick={joinSocialRoom} className="px-10 py-4 bg-white text-pink-600 font-black rounded-3xl shadow-2xl hover:scale-110 transition-transform cursor-pointer">ENTRAR NA SALA</button>
                    </div>
                    ) : (
