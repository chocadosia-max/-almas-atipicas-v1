        ...
        <div className="w-full max-w-7xl mx-auto h-[calc(100vh-60px)] flex flex-col p-2 overflow-hidden bg-transparent">
            ...
            <div className="flex flex-row items-center justify-between gap-4 mb-3 bg-white/30 backdrop-blur-2xl p-3 px-6 rounded-[2.5rem] border border-white/50 shadow-xl shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col"><span className="text-[10px] font-black text-pink-500 uppercase tracking-widest">Almas Atípicas</span><h1 className="text-xl font-black text-gray-800 leading-none">Rádio das Mães</h1></div>
                </div>
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'podcast' ? (
                    <motion.div key="stage-view" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex-1 flex flex-col gap-3 min-h-0 relative">
                        ...
                        <div onMouseMove={e => { const r = e.currentTarget.getBoundingClientRect(); mouseX.set(e.clientX - r.left - r.width/2); mouseY.set(e.clientY - r.top - r.height/2); }} className="flex-[2] min-h-[300px] rounded-[3.5rem] overflow-hidden shadow-2xl border-2 border-white/10 relative bg-black perspective-[1000px]">
                                ...
                                <div className="absolute inset-0 pointer-events-auto">
                                    ...
                                    {!hasJoinedLive ? (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-8 z-50">
                                            ...
                                            <button onClick={joinSocialRoom} className="px-14 py-5 bg-white text-pink-600 font-black rounded-3xl shadow-2xl hover:scale-110 transition-all text-sm uppercase">Sintonizar Sala</button>
                                        </div>
                                    ) : (
                            ...
                            <div className="absolute top-6 left-8 flex items-center gap-3 px-4 py-2 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10"> <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_green]" /> <span className="text-[10px] font-black text-white uppercase">{participants.length} On-line</span> </div>
                        </div>
                        <div className="bg-white/40 backdrop-blur-3xl rounded-[3rem] border border-white/60 p-4 flex flex-col gap-4 shadow-2xl shrink-0">
                            ...
                            <div className="flex gap-4 items-center">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={handleSendHeart} disabled={!selectedTargetId} className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl transition-all shadow-xl ${selectedTargetId ? 'bg-white text-pink-500' : 'bg-gray-100 opacity-50'}`}><Heart size={28} fill={selectedTargetId ? "currentColor" : "none"} /></button>
                            ...
                            <div ref={chatRef} className="h-28 overflow-y-auto px-5 py-3 bg-black/5 rounded-[2rem] space-y-2.5 no-scrollbar scroll-smooth shadow-inner border border-black/5"> {chatMessages.map(m => ( <div key={m.id} className="text-[12px] flex items-start gap-3 bg-white/50 backdrop-blur-sm p-3 rounded-2xl border border-white shadow-sm"><span className="text-pink-600 font-extrabold shrink-0">{m.name}:</span><span className="text-gray-800 font-bold break-words leading-snug">{m.text}</span>{m.audio && (<button onClick={() => new Audio(m.audio).play()} className="flex items-center gap-2 px-3 py-1 bg-pink-500 rounded-full text-white text-[9px] font-black uppercase ml-auto"><Volume2 size={12} /> Áudio</button>)}</div> ))} </div>
                        </div>
                    </motion.div>
                ) : (
                    /* Mural */
                    <motion.div key="mural-view" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex-1 flex flex-col gap-6 p-4 bg-pink-100/30 rounded-[3rem] border border-white/80 overflow-hidden relative">
                        <div className="flex flex-col gap-2 relative z-20"> <h2 className="text-3xl font-black text-gray-900 leading-tight">Mural de <span className="text-pink-500 italic">Desabafos</span></h2> <p className="text-sm text-gray-500 font-bold flex items-center gap-2"><Sparkles size={16} className="text-pink-400" /> Pendure sua voz aqui. Outras mães estarão ouvindo.</p> </div>
