...
const Petal = ({ id, onDone }: { id: number; onDone: () => void }) => {
  ...
  return (
    <motion.div initial={{ top: '-10%', left: `${startX}%`, opacity: 0, rotate: 0 }} animate={{ top: '110%', left: [`${startX}%`, `${startX + (Math.random() * 10 - 5)}%`], opacity: [0, 0.7, 0.7, 0], rotate: 360 }} transition={{ duration, delay, ease: 'linear' }} className="fixed pointer-events-none select-none text-2xl z-[99]">🌸</motion.div>
  );
};
...
const MuralCard = ({ card, onPlay }: any) => {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-white p-4 shadow-xl border-t-[12px] border-pink-500 rounded-sm w-full max-w-[280px]"
        >
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center shadow-inner border border-white">
               <Anchor size={14} className="text-pink-600" />
            </div>
            ...
            <div className="flex flex-col gap-4 mt-2">
                ...
                <div className="w-full bg-pink-50 h-[2px] rounded-full relative overflow-hidden">
                   <motion.div animate={{ x: ['-100%', '100%'] }} transition={{ repeat: Infinity, duration: 2, ease: "linear" }} className="absolute top-0 bottom-0 left-0 right-0 bg-pink-500/20" />
                </div>

                <button onClick={() => onPlay(card.audioData)} className="w-full py-4 rounded-xl flex items-center justify-center gap-2 font-black text-xs transition-all bg-gray-900 text-white hover:bg-pink-600 hover:scale-[1.02] shadow-lg">
                    <PlayCircle size={18} /> OUVIR DESABAFO
                </button>
                
                <div className="flex justify-between items-center px-1">
                    <div className="flex items-center gap-1 text-[10px] text-pink-500 font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                       <Sparkles size={10} /> ACOLHIDO POR 12 MÃES
                    </div>
                    <HeartIcon size={14} className="text-pink-200" fill="currentColor" />
                </div>
            </div>
        </motion.div>
    );
...
const RadioDasMaes = () => {
    ...
    const joinSocialRoom = useCallback(() => {
        ...
        channel
          .on('presence', { event: 'sync' }, () => {
             const users = Object.keys(channel.presenceState()).map(k => ({ ...(channel.presenceState()[k] as any)[0], id: k, isMe: k === userId.current })).filter(Boolean);
          ...
          .on('broadcast', { event: 'mural-pinned' }, ({ payload }) => {
             ...
             toast.info(`${payload.author} deixou um novo desabafo no Mural!`);
          })
            ...
            mr.onstop = () => {
              ...
              const r = new FileReader(); r.readAsDataURL(b); r.onloadend = () => {
                ...
                if (activeTab === 'podcast') {
                   ...
                   const newCard = { id: Date.now().toString(), author: myName, audioData: b64, time: 'Agora mesmo' };
    ...
    return (
        <div className="w-full max-w-7xl mx-auto h-[calc(100vh-60px)] flex flex-col p-2 overflow-hidden bg-transparent">
            ...
            {/* Header */}
            <div className="flex flex-row items-center justify-between gap-4 mb-3 bg-white/30 backdrop-blur-2xl p-3 px-6 rounded-[2.5rem] border border-white/50 shadow-xl shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-pink-500 uppercase tracking-widest">Almas Atípicas</span>
                        <h1 className="text-xl font-black text-gray-800 leading-none">Rádio das Mães</h1>
                    </div>
                </div>
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'podcast' ? (
                    <motion.div key="podcast-stage" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex-1 flex flex-col gap-3 min-h-0 relative">
                        {/* Interactive Stage */}
                        <div onMouseMove={e => { const r = e.currentTarget.getBoundingClientRect(); mouseX.set(e.clientX - r.left - r.width/2); mouseY.set(e.clientY - r.top - r.height/2); }} className="flex-[2] min-h-[300px] rounded-[3.5rem] overflow-hidden shadow-2xl border-2 border-white/10 relative bg-black perspective-[1000px]">
                                    ...
                                    {!hasJoinedLive ? (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-8 z-50">
                                            <div className="relative">
                                                <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }} transition={{ repeat: Infinity, duration: 3 }} className="absolute inset-0 bg-pink-500/30 rounded-full blur-3xl" />
                                                <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ repeat: Infinity, duration: 5 }} className="w-32 h-32 bg-white/10 rounded-[3rem] backdrop-blur-md flex items-center justify-center text-7xl relative shadow-2xl border border-white/20">🌙</motion.div>
                                            </div>
                                            <button onClick={joinSocialRoom} className="px-14 py-5 bg-white text-pink-600 font-black rounded-3xl shadow-[0_20px_40px_rgba(0,0,0,0.3)] hover:scale-110 active:scale-95 transition-all text-sm uppercase tracking-widest">Sintonizar Sala</button>
                                        </div>
                                    ) : (
                            ...
                            <div className="absolute top-6 left-8 flex items-center gap-3 px-4 py-2 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 shadow-xl">
                                <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_green]" />
                                <span className="text-[10px] font-black text-white uppercase tracking-widest">{participants.length} Conectadas</span>
                            </div>
                            <div className="absolute right-6 bottom-6 flex gap-2">
                               <button onClick={() => { if (!isAmbientAudioActive) ambientAudioRef.current?.play(); else ambientAudioRef.current?.pause(); setIsAmbientAudioActive(!isAmbientAudioActive); }} className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-xl bg-white/10 backdrop-blur-md border border-white/20 text-white hover:scale-110`}>
                                   {isAmbientAudioActive ? <Pause size={20} /> : <Play size={20} />}
                               </button>
                            </div>
                        </div>

                        {/* Control Hub */}
                        <div className="bg-white/40 backdrop-blur-3xl rounded-[3rem] border border-white/60 p-4 flex flex-col gap-4 shadow-2xl shrink-0">
                            <div className="flex items-center justify-between">
                                <div className="flex gap-2 p-1.5 bg-black/5 rounded-2xl border border-black/5 shadow-inner">
                                    <button onClick={() => { if (!hasJoinedLive) joinSocialRoom(); channelRef.current?.send({ type: 'broadcast', event: 'group-hug' }); }} className="flex items-center gap-2 px-5 py-2.5 hover:bg-white rounded-xl text-[10px] font-black text-gray-500 hover:text-pink-600 transition-all shadow-sm"><Users size={14} /> ABRAÇO COLETIVO</button>
                                    <button onClick={() => { if (!hasJoinedLive) joinSocialRoom(); channelRef.current?.send({ type: 'broadcast', event: 'petal-rain' }); }} className="flex items-center gap-2 px-5 py-2.5 hover:bg-white rounded-xl text-[10px] font-black text-gray-500 hover:text-pink-400 transition-all shadow-sm"><Leaf size={14} /> CHUVA DE PÉTALAS</button>
                                </div>
                                <div className="flex gap-1.5 px-3 py-2 bg-white/30 rounded-2xl border border-white/50 overflow-x-auto no-scrollbar max-w-[140px] sm:max-w-none">
                                    {moodEmojis.map(m => ( <button key={m} onClick={() => { if (!hasJoinedLive) joinSocialRoom(); channelRef.current?.send({ type: 'broadcast', event: 'update-mood', payload: { userId: userId.current, emoji: m } }); }} className="w-8 h-8 flex items-center justify-center hover:scale-150 transition-all text-sm filter drop-shadow-sm">{m}</button> ))}
                                </div>
                            </div>
                            <div className="flex gap-4 items-center">
                                <div className="flex-1 flex items-center bg-white/60 rounded-[2rem] px-2 py-1.5 border border-white focus-within:ring-4 ring-pink-500/10 transition-all shadow-xl">
                                    <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendMessage()} placeholder="Palavras que acolhem..." className="flex-1 bg-transparent px-6 py-2 text-sm text-gray-800 placeholder:text-gray-400 outline-none font-bold" />
                                    <button onClick={handleSendMessage} className="w-12 h-12 bg-pink-500 rounded-2xl flex items-center justify-center text-white mr-1 hover:scale-110 active:scale-95 transition-all shadow-xl"><Send size={20} /></button>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={handleSendHeart} disabled={!selectedTargetId} className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl transition-all shadow-xl ${selectedTargetId ? 'bg-white text-pink-500 scale-105' : 'bg-gray-100/50 text-gray-300 opacity-50 cursor-not-allowed'}`}><Heart size={28} fill={selectedTargetId ? "currentColor" : "none"} /></button>
                            ...
                            <div ref={chatRef} className="h-28 overflow-y-auto px-5 py-3 bg-black/5 rounded-[2rem] space-y-2.5 no-scrollbar scroll-smooth shadow-inner border border-black/5">
                                {chatMessages.length === 0 && <div className="text-[10px] text-gray-400 font-black uppercase py-4 text-center tracking-tighter opacity-30">A sala aguarda o som do seu acolhimento...</div>}
                                {chatMessages.map(m => ( <div key={m.id} className="text-[12px] flex items-start gap-3 animate-in fade-in slide-in-from-bottom-1 bg-white/50 backdrop-blur-sm p-3 rounded-2xl border border-white shadow-sm"><span className="text-pink-600 font-extrabold shrink-0">{m.name}:</span><span className="text-gray-800 font-bold break-words leading-snug">{m.text}</span>{m.audio && (<button onClick={() => new Audio(m.audio).play()} className="flex items-center gap-2 px-3 py-1 bg-pink-500 rounded-full text-white text-[9px] font-black uppercase hover:bg-pink-600 ml-auto transition-colors"><Volume2 size={12} /> Áudio</button>)}</div> ))}
                            </div>
                        </div>
                    </motion.div>
                ) : (
                    /* Creative Mural */
                    <motion.div key="mural-stage" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex-1 flex flex-col gap-6 p-4 bg-pink-100/30 rounded-[3rem] border border-white/80 overflow-hidden relative">
                        <div className="absolute top-0 left-0 right-0 h-40 bg-gradient-to-b from-white/80 to-transparent pointer-events-none z-10" />
                        
                        <div className="flex flex-col gap-2 z-20 relative">
                            <h2 className="text-3xl font-black text-gray-900 leading-tight">Mural de <span className="text-pink-500 italic">Desabafos</span></h2>
                            <p className="text-sm text-gray-500 font-bold flex items-center gap-2"><Sparkles size={16} className="text-pink-400" /> Pendure sua voz aqui. Outras mães estarão ouvindo.</p>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar pt-6 pb-20 relative z-20">
                             {desabafos.length === 0 ? (
                                 <div className="flex flex-col items-center justify-center h-full gap-6 opacity-30 select-none">
                                     <div className="w-40 h-40 bg-pink-200 rounded-full flex items-center justify-center text-7xl">🍃</div>
                                     <p className="text-xl font-black text-pink-900 uppercase tracking-widest">O mural está em silêncio...</p>
                                 </div>
                             ) : (
                                 <div className="flex flex-wrap justify-center gap-8 px-4">
                                     {desabafos.map(d => (
                                         <MuralCard key={d.id} card={d} onPlay={(audio: string) => { if (!hasJoinedLive) joinSocialRoom(); new Audio(audio).play(); }} />
                                     ))}
                                 </div>
                             )}
                        </div>

                        {/* Floating Record Button for Mural */}
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-50">
                            <button onClick={toggleRecording} className={`flex items-center gap-4 px-10 py-5 rounded-[2.5rem] font-black text-sm uppercase tracking-widest transition-all shadow-2xl ${isRecordingMsg ? 'bg-red-500 animate-pulse text-white scale-110' : 'bg-gray-900 text-white hover:bg-pink-600 hover:scale-105'}`}>
                                {isRecordingMsg ? <StopCircle size={22} /> : <Plus size={22} />}
                                {isRecordingMsg ? 'Gravando Desabafo...' : 'Pendurar Desabafo'}
                            </button>
                        </div>
                    </motion.div>
                )}
