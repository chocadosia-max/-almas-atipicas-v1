...
import {
  ...
  Users, Sparkles, Music, Leaf, Heart as HeartIcon
} from 'lucide-react';
...
const Firefly = () => {
  ...
  return (
    <motion.div animate={{ x: [0, Math.random() * 60 - 30, 0], y: [0, Math.random() * 60 - 30, 0], opacity: [0.1, 0.6, 0.1], scale: [1, 1.4, 1] }} transition={{ duration, repeat: Infinity, ease: 'easeInOut' }} className="absolute w-1.5 h-1.5 bg-yellow-300 rounded-full blur-[2px] pointer-events-none shadow-[0_0_8px_rgba(253,224,71,0.6)]" style={{ left: `${x}%`, top: `${y}%` }} />
  );
};
...
const FloatingBubble = ({ participant, index, total, isSelected, isReceivingHeart, isHugging, onClick, onBubbleRef }: any) => {
  ...
  return (
    <motion.div ref={el => onBubbleRef(participant.id, el)} initial={{ scale: 0, opacity: 0 }} animate={isReceivingHeart ? { scale: [heartScale, heartScale * 1.4, heartScale], opacity: 1 } : { scale: heartScale, opacity: 1, y: [0, -10, 0, 10, 0] }} transition={{ scale: { type: 'spring', stiffness: 260, damping: 20 }, y: { duration: 5 + index * 0.4, repeat: Infinity, ease: 'easeInOut' }, left: { type: 'spring', stiffness: 60, damping: 25 }, top: { type: 'spring', stiffness: 60, damping: 25 } }} className="absolute flex flex-col items-center gap-2 cursor-pointer z-30 group" style={{ left: `${cx}%`, top: `${cy}%`, transform: 'translate(-50%,-50%)' }} onClick={onClick}>
      ...
      {participant.playingAudio && ( <motion.div animate={{ scale: [1, 2.5, 1], opacity: [0.6, 0, 0.6] }} transition={{ duration: 0.8, repeat: Infinity }} className="absolute rounded-full bg-pink-400/40" style={{ width: bubblePx + 30, height: bubblePx + 30, top: -15, left: -15 }} /> )}
      {participant.mood && ( <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-4 -right-2 bg-white/90 backdrop-blur-sm shadow-xl rounded-full w-6 h-6 flex items-center justify-center text-xs border border-pink-100 z-40">{participant.mood}</motion.div> )}
      <motion.div whileHover={{ scale: 1.1, rotate: [0, -5, 5, 0] }} animate={{ width: bubblePx, height: bubblePx }} transition={{ type: 'spring' }} className="rounded-full flex items-center justify-center shadow-2xl border-4 border-white/40 ring-4 ring-pink-500/10 overflow-hidden relative bg-gradient-to-br from-pink-300 via-rose-500 to-rose-700">
        ...
        {participant.hasUnplayedAudio && ( <div className="absolute top-2 right-2 w-3 h-3 bg-red-50 border-2 border-white rounded-full animate-bounce" /> )}
      </motion.div>
      <div className="flex flex-col items-center">
        <span className="text-[10px] font-black text-white px-3 py-1 bg-black/40 backdrop-blur-md rounded-full truncate max-w-[90px] shadow-lg border border-white/5">{participant.isMe ? `VOCÊ` : participant.name.toUpperCase()}</span>
        {hearts > 0 && ( <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-[10px] font-black text-white bg-pink-500 px-2 py-0.5 rounded-full -mt-1 shadow-md border border-white/20">{hearts} ❤️</motion.span> )}
      </div>
    </motion.div>
  );
};
...
const RadioDasMaes = () => {
  ...
  useEffect(() => {
    ...
    audio.loop = true; audio.volume = 0.15; ambientAudioRef.current = audio;
    return () => { audio.pause(); };
  }, []);
  ...
  const joinSocialRoom = () => {
    ...
    channel
      .on('presence', { event: 'sync' }, () => {
        ...
        const users = Object.keys(state).map(key => { const u = (state[key] as any)?.[0]; return u ? { ...u, id: key, isMe: key === userId.current } : null; }).filter(Boolean);
        setParticipants(users);
      })
      .on('broadcast', { event: 'chat-msg' }, ({ payload }) => setChatMessages(prev => [...prev, payload]))
      .on('broadcast', { event: 'heart-event' }, ({ payload }) => {
        const targetEl = bubbleRefsMap.current.get(payload.targetId);
        let ox = window.innerWidth / 2, oy = window.innerHeight / 2;
        if (targetEl) { const r = targetEl.getBoundingClientRect(); ox = r.left + r.width / 2; oy = r.top + r.height / 2; }
        setHeartParticles(prev => [...prev, ...Array.from({ length: 14 }, (_, i) => ({ id: Date.now() + i, startX: ox, startY: oy }))]);
        setReceivingHeartId(payload.targetId);
      ...
      try {
        ...
        mr.onstop = () => {
          ...
          const r = new FileReader(); r.readAsDataURL(b); r.onloadend = () => {
            const b64 = r.result as string; channelRef.current?.send({ type: 'broadcast', event: 'audio-msg', payload: { id: userId.current, name: myName, audio: b64 } });
  ...
  return (
    <div className="w-full max-w-6xl mx-auto h-[calc(100vh-100px)] flex flex-col pt-2 pb-6 px-4 overflow-hidden">
      ...
      <div className="flex flex-row items-center justify-between gap-4 mb-3 bg-white/25 backdrop-blur-2xl p-3 px-6 rounded-[2rem] border border-white/40 shadow-xl shrink-0">
        ...
        {activeTab === 'podcast' ? (
          <motion.div key="stage" className="flex-1 flex flex-col gap-3 min-h-0 relative">
            ...
            <div onMouseMove={e => { const r = e.currentTarget.getBoundingClientRect(); mouseX.set(e.clientX - r.left - r.width/2); mouseY.set(e.clientY - r.top - r.height/2); }} className="flex-1 rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/20 relative bg-black perspective-[1000px]">
              ...
              <motion.div style={{ rotateX, rotateY, transformStyle: 'preserve-3d' }} className="absolute inset-0 w-full h-full flex items-center justify-center">
                ...
                {!hasJoinedLive ? (
                  <div className="flex flex-col items-center gap-6 z-50">
                    ...
                    <button onClick={joinSocialRoom} className="px-10 py-4 bg-white text-pink-600 font-black rounded-3xl shadow-2xl hover:scale-110 transition-transform">ENTRAR NA SALA</button>
                  </div>
                ) : (
            ...
            <div className="bg-white/30 backdrop-blur-3xl rounded-[2.5rem] border border-white/40 p-3 px-4 flex flex-col gap-3 shadow-2xl shrink-0">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex gap-2 p-1 bg-black/10 rounded-2xl border border-white/5 opacity-80 hover:opacity-100 transition-opacity">
                     ...
                     <button onClick={() => { if (!hasJoinedLive) joinSocialRoom(); channelRef.current?.send({ type: 'broadcast', event: 'petal-rain', payload: {} }); }} className="flex items-center gap-2 px-4 py-2 hover:bg-white/10 rounded-xl text-[10px] font-black text-pink-900/60 hover:text-pink-300 transition-all"><Leaf size={14} /> PÉTALAS</button>
                  </div>
                  <div className="flex gap-1.5 px-3 py-1.5 bg-white/10 rounded-2xl border border-white/20 overflow-x-auto no-scrollbar">
                     {moodEmojis.map(m => ( <button key={m} onClick={() => { if (!hasJoinedLive) joinSocialRoom(); channelRef.current?.send({ type: 'broadcast', event: 'update-mood', payload: { userId: userId.current, emoji: m } }); setParticipants(prev => prev.map(p => p.isMe ? { ...p, mood: m } : p)); }} className="w-7 h-7 flex items-center justify-center hover:scale-150 transition-all text-sm">{m}</button> ))}
                  </div>
                </div>
                <div className="flex gap-3 items-center">
                  <div className="flex-1 flex items-center bg-white/40 rounded-3xl px-1 border border-white/50 focus-within:border-pink-500/50 transition-all">
                     <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendMessage()} placeholder="Envie um acolhimento..." className="flex-1 bg-transparent px-5 py-3 text-sm text-gray-800 placeholder:text-gray-400 outline-none font-medium" />
                     <button onClick={handleSendMessage} className="w-10 h-10 bg-pink-500/10 rounded-2xl flex items-center justify-center text-pink-600 mr-1 hover:bg-pink-500 hover:text-white transition-all"><Send size={18} /></button>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={handleSendHeart} disabled={heartsSent.has(selectedTargetId)} className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl transition-all shadow-lg ${selectedTargetId && !heartsSent.has(selectedTargetId) ? 'bg-white text-pink-500 scale-110' : 'bg-white/20 opacity-40 cursor-not-allowed'}`}> {heartsSent.has(selectedTargetId) ? '✔️' : <HeartIcon size={24} fill="currentColor" />} </button>
                    <button onClick={toggleRecording} className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-xl ${isRecordingMsg ? 'bg-red-500 animate-pulse text-white' : 'bg-pink-500 text-white hover:scale-105'}`}> {isRecordingMsg ? <StopCircle size={22} /> : <Mic size={22} />} </button>
                  </div>
                </div>
                <div ref={chatRef} className="h-28 overflow-y-auto px-4 py-3 bg-white/40 rounded-3xl space-y-2 no-scrollbar scroll-smooth border border-black/5 shadow-inner">
                   ...
                   {chatMessages.map(m => ( <div key={m.id} className="text-[12px] flex items-start gap-2 animate-in fade-in slide-in-from-bottom-1 bg-white/30 backdrop-blur-sm p-2 rounded-2xl border border-white/50 shadow-sm"> <span className="text-pink-600 font-black shrink-0">{m.name}:</span> <span className="text-gray-900 font-medium break-words leading-tight">{m.text}</span> {m.audio && ( <button onClick={() => { const a = new Audio(m.audio); a.play(); }} className="flex items-center gap-1.5 px-3 py-1 bg-pink-500/20 rounded-full text-pink-700 hover:bg-pink-500/30 transition-all ml-auto"> <Volume2 size={12} /> <span className="text-[9px] font-black uppercase">Ouvir</span> </button> )} </div> ))}
                </div>
            </div>
          </motion.div>
        ) : (
          <div className="flex-1 overflow-y-auto pr-2 space-y-4 no-scrollbar"> {desabafos.map(d => ( <div key={d.id} className="bg-white p-5 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col gap-4"> <div className="flex justify-between items-center font-bold text-sm text-pink-600"> <div className="flex items-center gap-2">👩 Mãe {d.author}</div> <span className="text-[9px] text-gray-300 uppercase tracking-widest">{d.time}</span> </div> <button onClick={() => { const a = new Audio(d.audioData); a.play(); }} className="w-full py-4 rounded-2xl flex items-center justify-center gap-3 font-black bg-gray-50 text-gray-400 hover:bg-pink-500 hover:text-white transition-all"> <Play size={18} fill="currentColor" /> OUVIR DESABAFO </button> </div> ))} </div>
        )}
      </AnimatePresence>
    </div>
  );
