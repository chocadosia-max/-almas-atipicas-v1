                ...
                {activeTab === 'chat' ? (
                    <motion.div key="chat-stage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col min-h-[500px] gap-2">
                        ...
                        <div onMouseMove={e => { const r = e.currentTarget.getBoundingClientRect(); mouseX.set(e.clientX - r.left - r.width/2); mouseY.set(e.clientY - r.top - r.height/2); }} className="flex-1 min-h-[340px] rounded-[3rem] overflow-hidden shadow-2xl border-2 border-white/10 relative bg-black perspective-[1000px]">
                            ...
                            {/* LIVE CHAT OVERLAY */}
                            <div className="absolute md:left-6 left-2 md:top-12 md:bottom-6 bottom-2 h-[200px] md:h-auto md:w-[280px] right-2 md:right-auto bg-black/30 backdrop-blur-2xl rounded-[2rem] border border-white/10 z-50 flex flex-col overflow-hidden shadow-2xl">
                        ...
                        <div className="bg-white/40 backdrop-blur-3xl rounded-[2.5rem] border border-white/60 p-4 flex flex-col gap-2 shadow-xl shrink-0">
                            <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
                                <div className="flex gap-2 p-1 bg-black/5 rounded-2xl w-full md:w-auto justify-center shrink-0">
                                    <button onClick={() => { if (!hasJoinedLive) joinSocialRoom(); else { channelRef.current?.send({ type: 'broadcast', event: 'group-hug' }); setIsHugging(true); setTimeout(() => setIsHugging(false), 3500); } }} className="px-5 py-2 hover:bg-white rounded-xl text-[9px] font-black text-gray-500 hover:text-pink-600 transition-all flex items-center justify-center flex-1 md:flex-none gap-2 uppercase tracking-widest"><Users size={14} /> Abraço</button>
                                    <button onClick={() => { if (!hasJoinedLive) joinSocialRoom(); else { channelRef.current?.send({ type: 'broadcast', event: 'petal-rain' }); setPetals(v => [...v.slice(-39), Date.now()]); } }} className="px-5 py-2 hover:bg-white rounded-xl text-[9px] font-black text-gray-500 hover:text-pink-400 transition-all flex items-center justify-center flex-1 md:flex-none gap-2 uppercase tracking-widest"><Leaf size={14} /> Pétalas</button>
                                </div>
                                <div className="w-full md:flex-1 flex gap-1.5 px-3 py-2 bg-white/20 rounded-2xl overflow-x-auto no-scrollbar justify-start md:justify-center"> {moodEmojis.map(m => ( <button key={m} onClick={() => { if (!hasJoinedLive) return; channelRef.current?.send({ type: 'broadcast', event: 'update-mood', payload: { userId: userId.current, emoji: m } }); setParticipants(prev => prev.map(p => p.isMe ? { ...p, mood: m } : p)); }} className="w-8 h-8 flex items-center justify-center shrink-0 hover:scale-150 transition-all text-sm outline-none">{m}</button> ))} </div>
                                <div className="flex gap-3 items-center w-full md:w-auto">
                                    <div className="flex-1 md:w-[380px] flex items-center bg-white border-2 border-white/50 rounded-full px-2 py-1 shadow-2xl focus-within:ring-4 ring-pink-500/10 focus-within:border-pink-200 transition-all">
                                        <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendMessage()} aria-label="Digitar uma palavra de apoio" placeholder="Escreva uma palavra de apoio..." className="flex-1 bg-transparent px-4 py-2 text-xs w-full text-gray-800 placeholder:text-gray-400 outline-none font-bold min-w-0" />
                                        <button onClick={handleSendMessage} aria-label="Enviar mensagem" className="w-10 h-10 bg-gray-900 text-white rounded-full shrink-0 flex items-center justify-center hover:bg-pink-600 hover:scale-110 shadow-lg transition-all focus:outline-none focus-visible:ring-4 focus-visible:ring-pink-300"><Send size={16} /></button>
                                    </div>
                                    <button onClick={handleSendHeart} disabled={!selectedTargetId} aria-label="Enviar coração" className={`w-12 h-12 shrink-0 rounded-2xl flex items-center justify-center shadow-2xl transition-all focus:outline-none focus-visible:ring-4 focus-visible:ring-pink-300 ${selectedTargetId ? 'bg-white text-pink-500 scale-110 border border-pink-100' : 'bg-gray-100/50 opacity-40'}`}> <Heart size={20} fill={selectedTargetId ? "currentColor" : "none"} /> </button>
                                </div>
                            </div>
                        </div>
                    </motion.div>
